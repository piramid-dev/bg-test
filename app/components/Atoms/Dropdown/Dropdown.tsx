        ...
        <span
          className={classNames({
            'flex w-fit cursor-pointer flex-row items-center rounded-xl px-2 py-1 relative':
              true,
            'bg-dove-300': isHover && variant != 'dark',
            'border border-white text-white': variant === 'dark',
            'bg-white !text-black': isHover && variant === 'dark',
          })}
        >
          <span className="body-m font-sans min-w-[1.4em] text-right flex flex-row items-center">
            {label} {icon}
          </span>
          <CaretDown
            className={classNames({
              'ml-2 h-5 w-5 transition-all duration-300': true,
              '-rotate-180': isHover,
            })}
          />
            ...
            >
              {hasSubMenuPanel ? (
                <div className="absolute w-full pointer-events-none left-0">
                  ...
                  >
                    <div className="bg-dove-500 flex flex-row items-center justify-center border border-black rounded-xl overflow-hidden">
                      {children}
                    </div>
                  </div>
                </div>
                ...
                <div
                  className={classNames({
                    'absolute mt-2 border border-black rounded-xl overflow-hidden':
                      true,
                    'right-0': rightAlign,
                  })}
                >
